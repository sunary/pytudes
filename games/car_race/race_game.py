...
class Car(object):
    ...
    def __init__(self, start, width, height):
        ...
        self.relative_vertices = [[self.width/2, -self.height/2],
    ...
    def draw(self, canvas):
        ...
        canvas.create_oval(self.mid[0] - self.r_sight,
                           self.mid[1] - self.r_sight,
                           self.mid[0] + self.r_sight,
                           self.mid[1] + self.r_sight,
                           outline='red')
    ...
    def cal_vertext(self):
        for i in range(len(self.vertexs)):
            x = self.relative_vertices[i][0]
            y = self.relative_vertices[i][1]
            ...
            for i in range(len(self.car.sights)):
                for wall in self.walls:
                    ...
                    for point in points_collision:
                        if point:
                            self.collision_sight[i] = min(
                                self.collision_sight[i], math.sqrt(
                                    ((self.car.mid[0] - point[0])**2) + (self.car.mid[1] - point[1])**2)/self.car.r_sight)
