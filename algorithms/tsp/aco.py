...
class ACOAlgorithm(object):
    ...
    def process(self):
        ...
        for i in range(self.num_ant):
            if len(self.ant[i].trace) == self.size:
                print 'trace {}'.format(self.ant[i].trace)
                break
            ...
            for i in range(len(self.ant)):
                ...
                for e in range(self.size):
                    if e not in self.ant[i].trace and self.pheromones[self.ant[i].get_position()][e] > 0:
                        available_edge += (2.0 - self.distance[self.ant[i].get_position()][e]) * \
                ...
                for e in range(self.size):
                    if e not in self.ant[i].trace and self.pheromones[self.ant[i].get_position()][e] > 0:
                        prob_edge += (2.0 - self.distance[self.ant[i].get_position()][e]) * \
                                     self.pheromones[self.ant[i].get_position()][e]/available_edge
                        last_e = e
    ...
    def _finish_travel(self):
        ...
        if self.remain_path > 2*(self.size - 1):
            for i in range(len(self.pheromones)):
                for j in range(len(self.pheromones[0])):
                    if self.pheromones[i][j] <= THRESHOLD:
                        self.pheromones[i][j] = -1.0
        else:
            min_pheromones = INF
            for i in range(len(self.pheromones)):
